...
export async function commitImport(env: Env, ownerUid: string, sessionId: string) {
  ...
  statements.push(env.DB.prepare("UPDATE sync_accounts SET sync_epoch=?,revision=?,updated_at=? WHERE firebase_uid=? AND revision=?").bind(nextEpoch, session.record_count, now, ownerUid, session.base_revision));
